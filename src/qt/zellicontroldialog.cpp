...
#include "ui_zellicontroldialog.h"
...
std::list<std::string> ZElliControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZElliControlDialog::listMints;

ZElliControlDialog::ZElliControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZElliControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZElliControlDialog::~ZElliControlDialog()
{
    delete ui;
}

void ZElliControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZElliControlDialog::updateList()
{
...
void ZElliControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZElliControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZElli_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZElliControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZElliControlDialog::GetSelectedMints()
{
...
void ZElliControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
