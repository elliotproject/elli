...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ELLI);
    ...
    if (!settings.contains("nAnonymizeElliAmount"))
        settings.setValue("nAnonymizeElliAmount", 1000);

    nAnonymizeElliAmount = settings.value("nAnonymizeElliAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeElliAmount"))
        SoftSetArg("-anonymizeelliamount", settings.value("nAnonymizeElliAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in elli.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeElliAmount:
            return QVariant(nAnonymizeElliAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeElliAmount:
            nAnonymizeElliAmount = value.toInt();
            settings.setValue("nAnonymizeElliAmount", nAnonymizeElliAmount);
            emit anonymizeElliAmountChanged(nAnonymizeElliAmount);
            break;
        case CoinControlFeatures:
